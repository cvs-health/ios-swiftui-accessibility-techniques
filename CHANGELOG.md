...
- HTML report FIXED CODE abbreviated container form (`HStack { // … } .modifier()`) no longer shows original source context lines after the modifier. Previously, lines that followed the `{` in the snippet (e.g., `Image(...)` at line 74) appeared below the modifier, making the placement look ambiguous. The abbreviated block now ends cleanly after the modifier.
