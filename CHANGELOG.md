...
- HTML report "FIXED CODE" block was missing for `Button`, `Toggle`, `ScrollView`, `List`, and any other view using a trailing closure. The `generateCorrectedSnippet` guard that suppresses the preview for `NavigationStack`/`NavigationView` lines was incorrectly matching any line ending in `{` (i.e., all trailing-closure views). Narrowed the guard to only suppress lines beginning with `NavigationStack` or `NavigationView`, restoring FIXED CODE previews for all other view types.
