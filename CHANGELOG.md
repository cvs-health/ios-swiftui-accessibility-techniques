...
- HTML report FIXED CODE snippet window is now expanded to 5 context lines after the flagged line when the flagged line opens a trailing closure (ends with `{`). This ensures container children (e.g., Image and Text inside an HStack) are visible in both the CURRENT CODE and FIXED CODE blocks.
