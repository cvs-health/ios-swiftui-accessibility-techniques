...
- **New technique page: Accessibility Custom Content** (`AccessibilityCustomContentView.swift`, `Documentation/AccessibilityCustomContent.md`). Covers `.accessibilityCustomContent` for exposing supplementary details that are **not** shown as text on screen, surfaced to VoiceOver through the More Content rotor. The page leads with the constraint that matters most: custom content never replaces the accessible name. Moving visible text out of the name and into custom content means VoiceOver stops speaking it on focus and speech input users cannot target what they see, failing WCAG 2.5.3 Label in Name. Three good examples: a prescription card that lets the `Button` build its name from its visible text and uses custom content only for the refill count and prescriber that the compact card does not print; a progress ring using `.accessibilityElement(children: .combine)` to keep the visible "72%" in the name plus `importance: .high` for a comparison that is not drawn anywhere; and a list that reuses an `AccessibilityCustomContentKey` with a stable `id` so the same detail keeps a consistent position in the rotor across rows. Three bad examples: `.accessibilityElement(children: .ignore)` plus a truncated `.accessibilityLabel()` that drops three visible lines into the rotor (2.5.3); essential "Prescription expired" status pulled out of the name into `.default` importance custom content, with a note that raising it to `.high` does not fix the missing name text; and status conveyed only by a color dot plus custom content, which fails WCAG 1.4.1 for sighted users and is invisible to Voice Control and Full Keyboard Access because custom content is VoiceOver-only. Documentation notes `.default` vs `.high` behavior, that a long visible text block requires a correspondingly long name rather than custom content, the unavailable `String`-label overload (wrap in `Text` or use a key), and iOS 15 availability. Registered in `Techniques.swift`, `ContentView.swift`, and the README documentation index.
