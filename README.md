...
This repo also includes **[a11y-check](#a11y-checker-a11y-check)**, a static analysis tool that scans your Swift/SwiftUI source code for accessibility issues — 35 rules across 19 WCAG 2.2 criteria, with scoring, auto-fix, and CI integration.
...
Static analysis for SwiftUI accessibility issues, mapped to WCAG 2.2 success criteria. **35 rules** across **19 WCAG criteria** with a **0–100 scoring system**. Run it on your Swift sources to find missing labels, incorrect traits, touch target size, and more. Supports auto-fix (`--fix`), SARIF output for GitHub code scanning, trend tracking, and per-view scoring.
