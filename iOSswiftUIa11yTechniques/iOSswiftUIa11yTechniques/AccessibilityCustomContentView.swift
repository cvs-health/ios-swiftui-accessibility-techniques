...
struct AccessibilityCustomContentView: View {
    ...
    var body: some View {
        ScrollView {
            VStack {
                Text("Use `.accessibilityCustomContent` to attach extra details to an element as labelled key and value pairs, without adding them to its accessible name. VoiceOver exposes them under the More Content rotor. Apple built the API for data-rich apps where an element carries more information than its accessible name should hold, so that the extra information can be delivered concisely and only when someone wants it. VoiceOver speaks `.default` importance content only when the user asks for it, and speaks `.high` importance content immediately.")
                    .padding(.bottom)
