        ...
        ScrollView {
            VStack {
                ...
                ForEach(badItems, id: \.self) { item in
                    ...
                    .onDrag {
                        badDraggingItem = item
                        return NSItemProvider(object: item as NSString)
                    }
                    .onDrop(of: [.text], delegate: ReorderDropDelegate(
                        item: item,
                        items: $badItems,
                        draggingItem: $badDraggingItem
                    ))
                }
                DisclosureGroup("Details") {
                    Text("The bad drag and drop example only supports touch-based drag and drop with no single-tap alternative. There are no visible move buttons for users who cannot perform drag gestures, and no `.accessibilityAction` custom actions for VoiceOver, Switch Control, or Full Keyboard Access users to reorder items. The a11y-check `draggable-missing-accessibility-action` rule (WCAG 2.5.7) flags `.onDrag()` used without `.accessibilityAction(named:)` on the same modifier chain.")
                }
